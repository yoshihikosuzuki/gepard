...
public class DotMatrix implements Serializable {
	...
	public int[] getSeq1Starts(){
		return seq1Starts;
	}
	public int[] getSeq2Starts(){
		return seq2Starts;
	}
	...
	public DotMatrix(byte[] s1, byte[] s2, String seq1n, int[] seq1s, int[] seq1ns, String seq2n, int[] seq2s, int[] seq2ns,
		...
		this(s1, s2, seq1n, seq2n, sa, params, submat, callback, ac, SAforSecondSeq);

		seq1Starts = new int[seq1s.length];
		for(int i = 0; i < seq1s.length; i++){
			if(seq1s[i] > params.seq1Start && seq1s[i] < params.seq1Stop){
				seq1Starts[i] = (seq1s[i] - params.seq1Start) / params.zoom;
			}
			else{
				seq1Starts[i] = 0;
			}
		}
		seq2Starts = new int[seq2s.length];
		for(int i = 0; i < seq2s.length; i++){
			if(seq2s[i] > params.seq2Start && seq2s[i] < params.seq2Stop){
				seq2Starts[i] = (seq2s[i] - params.seq2Start) / params.zoom;
			}
			else{
				seq2Starts[i] = 0;
			}
