...
public class Plotter {
	...
	public Plotter(DotMatrix dm, DrawTarget dp, ControlPanel cp) {
		this(dm, dp);
		this.cp = cp;
	}
	...
	public void redraw(Graphics2D g) {
		...
		if(cp == null || !cp.reversePlot()){
			...
			g.drawString(dm.getParameterSet().seq2Stop + "", XOFF - 17 - fontMetrics.stringWidth(dm.getParameterSet().seq2Stop + ""), YOFF + 4);
			g.drawString(dm.getParameterSet().seq2Start + "", XOFF - 17 - fontMetrics.stringWidth(dm.getParameterSet().seq2Start + ""), YOFF + dm.getHeight() + 2);
		...
		for(int start : dm.getSeq2Starts()){
			if(start > 0){
				if(cp == null || !cp.reversePlot()){
					g.drawLine(XOFF, start + YOFF, XOFF + dm.getWidth(), start + YOFF);
				}
